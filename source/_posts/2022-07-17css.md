...
- offset系列：`offsetTop`、`offsetLeft`、`offsetWidth`、`offsetHeight` <a href="https://www.jb51.net/article/219621.htm" target="_blank" >距离元素最近的一个具有定位的祖宗元素（relative，absolute，fixed），若祖宗都不符合条件，offsetParent为body</a>
- scroll系列： `scrollTop`、`scrollLeft`、`scrollWidth`、`scrollHeight` <a href="https://blog.csdn.net/muxi229/article/details/106849535" target="_blank" >方法返回或设置匹配元素的滚动条的偏移位置</a>
- client系列：`clientTop`、`clientLeft`、`clientWidth`、`clientHeight` clientTop、clientLeft获取的是上边框、左边框的宽度 <a href="https://developer.mozilla.org/zh-CN/docs/Web/API/Element/clientLeft" target="_blank" >见MDN</a>
- `width`、`height`
