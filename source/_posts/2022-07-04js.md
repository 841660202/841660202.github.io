  ...
  return new Promise((resolve, reject) => {
    ...
    promises.forEach((promise, index) => {
      promise
        .then((data) => {
          res.push(data);
          if (index === promises.length - 1) { // 所有的都没有异常
            resolve(res);
          }
        })
        .catch((err) => { // 捕获，直接返回
...
Promise.myRace = (promises) =>
  new Promise((resolve, reject) => { // 谁先有结果，返回谁
...
<a href="https://www.freesion.com/article/68121031859/" target="_blank" >使用PROMISE过程中RESOLVE或REJECT后，后面代码还会执行，默认加RETURN较妥</a>
